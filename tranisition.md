...
+ first/firstly, second/secondly, third/thirdly [adv]首先
...
- first off == first of all == at first [adv]首先
...
+ to begin with == to start with == for a start [adv]首先
...
- last but not least [adv]最後但同樣重要的
...
+ additionally == in addition (to sth) [adv]此外
...
+ moreover == furthermore == further == also [adv]此外
...
+ and what is more == and more than that == and another thing [adv]此外
...
- even more (than sth) 除..之外 == on top of sth [adv]除..之外
...
+ afterward(s) == and then == next [adv]接下去/然後
...
- as an illustration (of sth) [adv]舉..例來說
...
+ for example == for instance [adv]舉例來說
...
- for one thing ... (and another thing) [adv]其中一個理由(，而另一個理由)
...
+ concerning == regarding == apropos of == with regard/respect to == in respect of/to == with reference to == as for == about [adv]關於
...
+ in other words == (to) put (it) another way [adv]換句話說
...
+ that is (to say) == to be specific [adv]換句話說/確切地說
...
+ in a/one sense == in a way [adv]在某種意義上
...
- put simply == in brief == in a few words == in short [adv]簡而言之
...
- as a rule [adv]在多數情況下/通常 == generally == usually
...
- in all probability [adv]很可能 == possibly
	- In all probability, he will go home tomorrow.

## Emphasis 強調
- in fact == as a matter of fact [adv]事實上/實際上 
...
+ without (a) doubt == no doubt == undoubtedly == definitely == certainly == clearly == surely [adv]無疑地/確實
...
+ more importantly == above all (else) [adv]更重要地是
...
+ in particular == particularly == specifically = especially [adv]特別/尤其
...
- by no means [adv]決不/一點都不 == at no point == not by all means == not at all
...
+ **although** == **though** == **while** == **(as) much as** == **even though** [adv]雖然/儘管
...
+ albeit == in spite of (the fact that) == despite (the fact that) == even so [adv]即使
...
+ regardless (of sth) == anyway [adv]無論如何/無論..
...
+ however == still == yet == *be that as it may* == for all that == *nevertheless == nonetheless == none the less* [adv]然而/可是
...
+ on the contrary == rather [adv]恰恰相反
...
+ in/by contrast (to/with) [adv]與...成對比
...
+ conversely == oppositely == contrarily [adv]反過來說
...
- rather than == instead of == *as opposed to* == before [adv]而不是/而非
...
+ likewise == in the same way == similarly == also [adv]同樣地
...
## Cause
+ **in that** == **being that** == **since** == **inasmuch as** == **because** [adv]由於/因為
...
+ *in light of * == *on account of* == *by virtue of* == due to == by reason of == because of [adv]因為/歸因
...
+ considering (that) == seeing (that) == given (the fact that) [adv]考慮到/鑒於
...
+ in order to == with a view to N == in an effort to == in an attempt to [adv]為了
...
+ as a consequence (of sth) == in consequence (of sth) == as a result (of sth) [adv]作為..結果
...
+ *therefore == thereby == thus == accordingly == consequently == hence == for that reason == accordingly* [adv]所以/因此
...
- as mentioned earlier/previously [adv]如前所述
+ in conclusion == in summary == to conclude = to sum up == to summarize [adv]總結一下/總之/總的來說
...
+ on the whole == as a whole == taking everything into account/consideration == Overall, it may be said… [adv]總的來說
...
- in the event of/that [adv]如果..發生
...
+ **Granted that** = **Assuming that** [adv]假定/假使
...
- **as long as** == **so long as** = **provided that** [adv]只要
...
+ the more ... the more/less [adv]越...就越...
...
+ no sooner ... than [adv]剛...就... == used to show that one thing happens immediately after another thing
...
- at that point in time [adv]那時 == At a particular moment in the past
	- It gets people to share what their priorities in life are at that point in time
- along the way [adv]在這個過程中/在這期間 == during the time that something is happening or that you are doing something
...
- at that point in time == At a particular moment in the past
	- It gets people to share what their priorities in life are at that point in time
- **as soon as** == **when** == **once** == **by the time** == **the second (that)** [adv]一...就.../一旦
...
- **no sooner/scarcely/hardly ... than ...** [adv]當/就
...
- at that time == at the same time == in the meantime == simultaneously == concurrently == at once == meanwhile [adv]同時
...
- during
- so far [adv]到目前為止 = up to this time
